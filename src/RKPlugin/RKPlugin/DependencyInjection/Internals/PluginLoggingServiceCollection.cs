...
internal static class PluginLoggingServiceCollection
{
    static readonly string BaseType = "Microsoft.Extensions.DependencyInjection.LoggingServiceCollectionExtensions,Microsoft.Extensions.Logging";

    public static object? AddLogging(this object? services)
    {
        var type = Type.GetType(BaseType);
        var methodInfo = type?.GetMethods().Where(x =>
            x.Name == nameof(AddLogging)
